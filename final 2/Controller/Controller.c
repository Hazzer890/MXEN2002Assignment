...
int main(void)
{
	...
	while(1)
	{
		...
		{
			if(adc_read(1)/4 > 253) {sendDataByte1 = 253;} else {sendDataByte1 = adc_read(1)/4;} // update bytes to new values if they are not greater than 253
			if(adc_read(2)/4 > 253) {sendDataByte2 = 253;} else {sendDataByte2 = adc_read(2)/4;}
			if(adc_read(3)/4 > 253) {sendDataByte3 = 253;} else {sendDataByte3 = adc_read(3)/4;}
			if(adc_read(4)/4 > 253) {sendDataByte4 = 253;} else {sendDataByte4 = adc_read(4)/4;}
			sendDataByte5 = 0;
